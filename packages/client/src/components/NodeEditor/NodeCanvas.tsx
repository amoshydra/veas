...
function makeBaseNodeComponent(type: keyof typeof NODE_DEFINITIONS) {
  return function GenericNode(props: any) {
    ...
    return (
      <div
        className={`rounded-lg border-2 ${statusBorder} bg-slate-800 shadow-lg min-w-[200px] relative ${props.selected ? "ring-2 ring-blue-400" : ""}`}
      ...
      >
        ...
        <div className="nodrag cursor-default px-3 py-2 space-y-2">
          <div className="text-xs text-slate-400">{getConfigSummary(type, config)}</div>
        </div>
...
function getConfigSummary(
  ...
  switch (type) {
    case "filter":
      return "Adjustments";
    case "speed":
      return `${config.speed ?? 1}x`;
    case "rotate":
      return `${config.angle ?? 90}°`;
    case "flip":
    ...
    case "pip":
      return "PiP overlay";
    case "audioExtract":
      return config.audioCodec || "mp3";
    case "gif":
      return `${config.fps ?? 10}fps`;
    case "reverse":
      return config.videoOnly ? "Video only" : "Video+Audio";
    case "loop":
      return `${config.count ?? 2}× loop`;
    case "subtitle":
      return "Burn subtitles";
    default:
      return NODE_DEFINITIONS[type]?.label || "Configure";
  }
}
...
function FlowContent({
  ...
  const onConnect = useCallback(
    (params: Connection) => {
      ...
      store.addEdge(newEdge);
      setEdges((eds) => {
        let updated = eds.filter((e) => !(e.target === target && e.targetHandle === targetHandle));
        return addEdge(
